...
def sanitize_string_input(input_string):
  # List of potentially dangerous characters or patterns to remove
    ...
    sanitized_string = re.sub(r"\b(SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|EXEC|UNION|FROM|WHERE|JOIN|DATABASE)\b", "", sanitized_string, flags=re.IGNORECASE)
...
def sanitize_pool_input(input_pool):
  try:  
    ...
    if ((input_pool["probability_legendary"] + input_pool["probability_epic"] + input_pool["probability_rare"] + input_pool["probability_common"]) != 1):
        return False
