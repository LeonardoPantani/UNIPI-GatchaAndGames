...
class GachaPlayer(HttpUser):
    ...
    def on_start(self):
        ...
        self.credentials = {"username": username, "email": f"{username}@example.com", "password": "test1234567"}
    ...
    def pull_gacha(self):
        ...
        with self.client.post(
            f"/gacha/pull/{random.choice(pool_ids)}", verify=False, catch_response=True, name="/gacha/pull"
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
