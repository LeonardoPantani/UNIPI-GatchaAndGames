...
def delete_user_transactions(session=None, uuid=None):
    ...
    try:
        ...
        def delete_transactions():
            ...
            MOCK_BUNDLESTRANSACTIONS = {k: v for k, v in MOCK_BUNDLESTRANSACTIONS.items() if k[0] != uuid}
            MOCK_INGAMETRANSACTIONS = {k: v for k, v in MOCK_INGAMETRANSACTIONS.items() if k[0] != uuid}
    ...
    try:
        ...
        def get_user_transactions():
            if history_type == "bundle":
                transactions = [v for k, v in MOCK_BUNDLESTRANSACTIONS.items() if k[0] == uuid]
            else:
                transactions = [v for k, v in MOCK_INGAMETRANSACTIONS.items() if k[0] == uuid]
...
def insert_ingame_transaction(session=None, uuid=None, current_bid=None, transaction_type=None):
    ...
    try:
        ...
        def insert_transaction():
            ...
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            MOCK_INGAMETRANSACTIONS[(uuid, timestamp)] = (uuid, current_bid, transaction_type, timestamp)
