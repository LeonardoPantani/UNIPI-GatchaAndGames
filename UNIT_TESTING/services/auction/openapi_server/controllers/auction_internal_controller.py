...
def create_auction(auction=None, session=None):
    if not auction:
        ...
        end_time = str(auction.get("end_time"))
    ...
    try:
        ...
        def insert_auction():
            ...
            MOCK_AUCTIONS[uuid] = (uuid, item_id, starting_price, 0, None, end_time)
        ...
        def get_auction_list():
            ...
            if status == "open":
                filtered_auctions = {
                    ...
                    if datetime.strptime(v[5], "%Y-%m-%d %H:%M:%S") > datetime.now()
                }
            else:
                filtered_auctions = {
                    k: v
...
def remove_by_item_uuid(request_body=None, session=None):
    ...
    try:
        ...
        def delete_auctions():
            ...
            MOCK_AUCTIONS = {k: v for k, v in MOCK_AUCTIONS.items() if v[1] not in request_body}
