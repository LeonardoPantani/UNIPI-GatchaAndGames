...
def delete_user_inventory(uuid=None, session=None):
    ...
    try:
        ...
        def delete_user_items():
            ...
            for item_uuid, item_data in list(
                MOCK_INVENTORY_DATA.items()
            ):  # Use list() for safe iteration during deletion
...
def get_inventory_items_by_owner_uuid(session=None, uuid=None, page_number=None):
    ...
    try:
        ...
        def get_user_inventory_items():
            ...
            filtered_items = [
                item_data for item_data in MOCK_INVENTORY_DATA.values() if item_data["owner_uuid"] == uuid
            ]
