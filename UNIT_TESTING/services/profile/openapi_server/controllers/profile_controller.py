...
def delete_profile():
    ...
    try:
        ...
        def reset_current_bidder():
            for k, v in MOCK_AUCTIONS.items():
                if v[3] == session["uuid"]:
                    MOCK_AUCTIONS[k] = (v[0], 0, None, None, v[4])
        ...
        def delete_auth_user():
            global MOCK_USERS
            MOCK_USERS = {k: v for k, v in MOCK_USERS.items() if k != session["uuid"]}
    ...
    try:
        ...
        if edit_request.email:
            ...
            def update_email():
                ...
                MOCK_USERS[user_uuid] = (
                    email,
                    MOCK_USERS[user_uuid][1],
                    MOCK_USERS[user_uuid][2],
                )
        ...
        if edit_request.username:
            ...
            @circuit_breaker
            def update_username():
                return edit_username(uuid=user_uuid, username=username)
