...
def check_pending_pvp_requests(ban_user_profile_request=None):
    ...
    try:
        ...
        # Format the results
        formatted_results = [
            {"pvp_match_uuid": match_uuid, "from": player_1_uuid} for match_uuid, player_1_uuid, _ in results
        ]
...
def finalize_pvp_request_sending(pv_p_request_full=None):
    ...
    try:
        ...
        def make_request_to_db():
            ...
            """
            cursor.execute(
                insert_query, (match_uuid, player1_uuid, user_uuid, None, None, jsonify(teams).get_data(as_text=True))
            )
            connection.commit()
...
def get_gacha_stat(get_gacha_stat_request=None):
    ...
    try:
        ...
        response = {"player1_stat": gachas[0], "player2_stat": gachas[1]}
