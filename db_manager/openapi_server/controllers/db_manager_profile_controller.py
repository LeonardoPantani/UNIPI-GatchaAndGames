...
def edit_user_info(edit_user_info_request=None):
    ...
    try:
        ...
        def make_request_to_db():
            ...
            cursor.execute("SELECT 1 FROM users WHERE uuid = UUID_TO_BIN(%s)", (user_uuid,))
            ...
            if email:
                cursor.execute("UPDATE users SET email = %s WHERE uuid = UUID_TO_BIN(%s)", (email, user_uuid))
                updates += cursor.rowcount

            if username:
                cursor.execute("UPDATE profiles SET username = %s WHERE uuid = UUID_TO_BIN(%s)", (username, user_uuid))
                updates += cursor.rowcount
...
def get_user_hash_psw(ban_user_profile_request=None):
    ...
    try:
        ...
        def make_request_to_db():
            connection = get_db()
            cursor = connection.cursor()
            cursor.execute("SELECT password FROM users WHERE uuid = UUID_TO_BIN(%s)", (user_uuid,))
...
def get_user_info(ban_user_profile_request=None):
    ...
    try:
        ...
        user = User(  # aggiungere pvp score e currency
            id=result[0], username=result[1], email=result[2], joindate=result[3]
        )
